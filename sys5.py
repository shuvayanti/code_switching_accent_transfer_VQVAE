...
class Model(nn.Module) :
    ...
    def forward_codedump(self, unpadded, padded):
        ...
        with torch.no_grad() :
            ...
            #print('continuous phn done', continuous_phn)
            continuous_spk = self.encoder_spk(padded)
            #print('continuous spk done', continuous_spk)
            tap_output_spk = self.tap_spk(continuous_spk)
            #print('tap spk', tap_out_spk)
...
#        return phn_codes, phn_vecs, spk_codes, spk_vecs
    ...
    def forward_custom(self, unpadded, padded, deterministic=False, use_half=False, verbose=True):
        self.eval()
        with torch.no_grad() :
            print('got into custom')
