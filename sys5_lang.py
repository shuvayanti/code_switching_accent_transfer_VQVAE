...
class Model(nn.Module) :
    def __init__(self, lang_dims, nspeakers, rnn_dims, fc_dims, global_decoder_cond_dims, upsample_factors, normalize_vq=False,   #use lang_dims when using siwis model
        ...
        # output of encoder_spk -> TAP+FF -> VQ
        self.tap_spk = TemporalPooling_spk(128)
        self.tap_phone = TemporalPooling_phn(128)
        
        # initialize a speaker classifier here
        # a linear layer w a softmax or something like that
        ...
        # print("Overtone init dims", rnn_dims, fc_dims, 128, global_decoder_cond_dims)
        # 1024 1024 128 99
        self.frame_advantage = 15
        self.num_params()
        
        # check the global conditioning dims
    ...
    def forward(self, x, samples, lang):
        ...
        print("phone VQ input", continuous_phn.unsqueeze(2).shape)
        print("speaker TAP input", continuous_spk.unsqueeze(2).shape)
        
        # gradient reversal layer here
        phn_GRL = self.grl(continuous_phn)
        print("phn_GRL", phn_GRL.shape)
        ...
        print("speaker VQ input:", tap_output_spk.unsqueeze(2).size())
        print("phone VQ input:", tap_output_phn.unsqueeze(2).size())

        # speaker classifiers here
        spk_predict_spk = self.speaker_classifier1(emb_spk)
        spk_predict_phn = self.speaker_classifier2(emb_phn)

        discrete, vq_pen, encoder_pen, entropy = self.vq(continuous_phn.unsqueeze(2))
        discrete_spk, vq_pen_spk, encoder_pen_spk, entropy_spk = self.vqspk(tap_output_spk.unsqueeze(2))
        
        code_x = discrete.squeeze(2)
        print("phone to waveRNN", code_x.shape)
        discrete_spk = discrete_spk.squeeze(2)
        print("discrete_spk", discrete_spk.shape)
        global_spk = discrete_spk.squeeze(1)
        print("global_spk", global_spk.shape)
        print("lang", lang.shape)
        global_codes = torch.cat((global_spk, lang) , dim=1 )
        print("speaker to waveRNN", global_spk.shape)
        print("lang to waveRNN", lang.shape)
        print("wave input to waveRNN", x.shape)
        #print("\nVQ-phn", phn_codes)
        #print("VQ-spk", spk_codes)
    ...
    def forward_generate(self, x, A, B, deterministic=False, use_half=False, verbose=False):
        if use_half:
            A = A.half()
        self.eval()
        with torch.no_grad() :
            print("encoder input", x.size())
            print("speaker A encoder input", A.size())
            print("speaker B encoder input", B.size())
            continuous = self.encoder(x)
            continuous_spk = self.encoder_spk(A)
            continuous_spk_xfer = self.encoder_spk(B)
            print("phone VQ input:", continuous.unsqueeze(2).size())
            print("speaker VQ input (A):", continuous_spk.unsqueeze(2).size())
            print("speaker VQ input (B):", continuous_spk_xfer.unsqueeze(2).size())

            
            tap_spk = self.tap_spk(continuous_spk)
            tap_spk_xfer = self.tap_spk(continuous_spk_xfer)
            print("speaker TAP output (A)", tap_spk.shape)
            print("speaker TAP output (B)", tap_spk_xfer.shape)

            
            discrete, vq_pen, encoder_pen, entropy = self.vq(continuous.unsqueeze(2))
            discrete_spk, vq_pen_spk, encoder_pen_spk, entropy_spk = self.vqspk(tap_spk.unsqueeze(2))
            discrete_spk_xfer, _, _, _ = self.vqspk(tap_spk_xfer.unsqueeze(2))
            code_x = discrete.squeeze(2)
            print("phone to waveRNN", code_x.shape)
            discrete_spk = discrete_spk.squeeze(2)
            print("speaker VQ output before upsample (A)", discrete_spk.shape)
            discrete_spk_xfer = discrete_spk_xfer.squeeze(2)
            print("speaker VQ output before upsample (B)", discrete_spk_xfer.shape)
            
            global_spk = discrete_spk.squeeze(1)
            global_spk_xfer = discrete_spk_xfer.squeeze(1)
            print("speaker to waveRNN (A)", global_spk.shape)
            print("speaker to waveRNN (B)", global_spk_xfer.shape)

            code_phone = torch.cat((code_x, code_x), dim=0)
            code_spk = torch.cat((global_spk, global_spk_xfer), dim=0)
            print("phone codes to waveRNN", code_phone.shape)
            print("speaker codes to waveRNN", code_spk.shape)
            output = self.overtone.generate(code_phone, code_spk)
        self.train()
        return output, discrete_spk, discrete_spk_xfer, vq_pen.mean(), encoder_pen.mean(), entropy, vq_pen_spk.mean(), encoder_pen_spk.mean(), entropy_spk 



    def forward_gen(self, x, A, langs, deterministic=False, use_half=False, verbose=False):
        self.eval()
        with torch.no_grad() :
            print("encoder input", x.size())
            print("speaker A encoder input", A.size())
...
#            discrete_spk, vq_pen_spk, encoder_pen_spk, entropy_spk, spk_codes = self.vqspk(tap_spk.unsqueeze(2))
            discrete, vq_pen, encoder_pen, entropy = self.vq(continuous.unsqueeze(2))
            discrete_spk, vq_pen_spk, encoder_pen_spk, entropy_spk = self.vqspk(tap_spk.unsqueeze(2))
            code_x = discrete.squeeze(2)
            print("phone to waveRNN", code_x.shape)
            ...
            global_spk = discrete_spk.squeeze(1)
            print("speaker to waveRNN (A)", global_spk.shape)
            print("lang", langs.shape)
            global_codes = torch.cat((global_spk, langs) , dim=1 )
            output = self.overtone.generate(code_x, global_codes)
        return output


    def forward_audio_from_phonecodes(self, A, phncodes, L, deterministic=False, use_half=False, verbose=False):
        self.eval()
        with torch.no_grad() :
            print("speaker A encoder input", A.size())
            continuous_spk = self.encoder_spk(A)
            ...
            discrete = self.vq.vecs_from_codes(phncodes)
            print("discrete", discrete.shape)
            code_x = discrete.unsqueeze(0)
            print("phone to waveRNN", code_x.shape)
            discrete_spk = discrete_spk.squeeze(2)
            print("speaker VQ output before upsample (A)", discrete_spk.shape)
            global_spk = discrete_spk.squeeze(1)
            print("CODEGEN: code_x", code_x.shape)
            print("CODEGEN: global_spk", global_spk.shape)
            global_codes = torch.cat((global_spk, L) , dim=1 )
            output = self.overtone.generate(code_x, global_codes)
        return output


    def forward_audio_from_only_codes(self, phncodes, L, spk_code):
        self.eval()
        with torch.no_grad() :
            discrete = self.vq.vecs_from_codes_normed(phncodes)
            discrete_spk = self.vqspk.vecs_from_codes_normed(spk_code) #normed it.
            print("discrete", discrete.shape)
            code_x = discrete.unsqueeze(0)
            #code_x = discrete.squeeze(2)
            print("phone to waveRNN", code_x.shape)
            global_spk = discrete_spk
            #discrete_spk = discrete_spk.squeeze(2)
            #global_spk = discrete_spk.squeeze(1)
            print("CODEGEN: code_x", code_x.shape)
            print("CODEGEN: global_spk", global_spk.shape)
            global_codes = torch.cat((global_spk, L) , dim=1 )
            #print("global code = ", global_codes)
            #print("global speaker =", global_spk)
            output = self.overtone.generate(code_x, global_codes)
        return output
    ...
    def forward_audio_from_phonecodes_prepend(self, A, x, phncodes, L, deterministic=False, use_half=False, verbose=False):
        self.eval()
        with torch.no_grad() :
            print("speaker A encoder input", A.size())
            continuous_phn = self.encoder(x)
            continuous_spk = self.encoder_spk(A)
            print("speaker VQ input (A):", continuous_spk.unsqueeze(2).size())            
            tap_spk = self.tap_spk(continuous_spk)
            print("speaker TAP output (A)", tap_spk.shape)
            ...
            prepend = self.vq.vecs_from_codes(phncodes)
            print("prepend", prepend.shape)
            print("discrete", discrete.shape)
            
            code1 = prepend.unsqueeze(0)
            code2 = discrete.squeeze(2)
            print("code1", code1.shape)
            print("code2", code2.shape)
            code_x = torch.cat((code1, code2), dim=1)
    ...
    def forward_validate(self, x, A, lang, deterministic=False, use_half=False, verbose=False):
        ...
        with torch.no_grad() :
            ...
            continuous_spk = self.encoder_spk(A)
            print("phone VQ input:", continuous_phn.unsqueeze(2).size())

            tap_output_phn = self.tap_phone(continuous_phn)
            tap_output_spk = self.tap_spk(continuous_spk)
            emb_spk = tap_output_spk.permute(0, 2, 1).squeeze(2)
            emb_phn = tap_output_phn.permute(0, 2, 1).squeeze(2)
            print("speaker VQ input (A):", tap_output_spk.unsqueeze(2).size())

            # speaker classifiers here
            spk_predict_spk = self.speaker_classifier1(emb_spk)

            # gradient reversal layer here
            print("emb_phn", emb_phn.shape)
    ...
    def forward_validate2(self, x, samples, lang):
        self.eval()
        with torch.no_grad() :
            continuous_phn = self.encoder(samples)
            continuous_spk = self.encoder_spk(samples)
            ...
            global_spk = discrete_spk.squeeze(1)
            global_codes = torch.cat((global_spk, lang) , dim=1 )
            out = self.overtone(x, code_x, global_codes)
        self.train()
        return out

    
    def forward_codedump(self, unpadded, padded):
        self.eval()
        with torch.no_grad() :
            continuous_phn = self.encoder(unpadded)
            continuous_spk = self.encoder_spk(padded)
            tap_output_spk = self.tap_spk(continuous_spk)
            phn_codes = self.vq.forward_custom(continuous_phn.unsqueeze(2))
            spk_codes = self.vqspk.forward_custom(tap_output_spk.unsqueeze(2))
            phn_vecs, pen1, pen2, l1 = self.vq(continuous_phn.unsqueeze(2))
            spk_vecs, pen3, pen4, l2 = self.vqspk(tap_output_spk.unsqueeze(2))
            all_phn = self.vq.forward_codebook_normed(512, continuous_phn.unsqueeze(2))
            all_spk = self.vqspk.forward_codebook_normed(256, tap_output_spk.unsqueeze(2))
#        return phn_codes, phn_vecs
#        return phn_codes, phn_vecs, spk_codes, spk_vecs
        return phn_codes, phn_vecs, spk_codes, spk_vecs, all_phn, all_spk


    
    
    def forward_custom(self, unpadded, padded, deterministic=False, use_half=False, verbose=False):
        self.eval()
        with torch.no_grad() :
            print("unpadded", unpadded.shape)
            print("padded", padded.shape)
            continuous_phn = self.encoder(unpadded)
            print("phone encoder", continuous_phn.unsqueeze(2).shape)
            continuous_spk = self.encoder_spk(padded)
            tap_output_spk = self.tap_spk(continuous_spk)
            phn_codes = self.vq.forward_custom(continuous_phn.unsqueeze(2))
#            all_phn_vq_vecs = self.vq.forward_get_all_codes(continuous_phn.unsqueeze(2), 512)
#            all_spk_vq_vecs = self.vqspk.forward_get_all_codes(tap_output_spk.unsqueeze(2), 256)
            discrete_phn, pen1, pen2, l1 = self.vq(continuous_phn.unsqueeze(2))
            discrete_spk, pen3, pen4, l2 = self.vqspk(tap_output_spk.unsqueeze(2))
        self.train()
        return phn_codes, discrete_phn#, discrete_spk, all_phn_vq_vecs, all_spk_vq_vecs
#        return phn_codes, discrete_phn, discrete_spk, all_phn_vq_vecs, all_spk_vq_vecs

    
    
    def forward_embeddings(self, A, deterministic=False, use_half=False, verbose=False):
        self.eval()
        with torch.no_grad() :
            print("A", A.size())
            continuous_spk = self.encoder_spk(A)
            print("speaker VQ input (A):", continuous_spk.unsqueeze(2).size())
            tap_spk = self.tap_spk(continuous_spk)
            print("speaker TAP output", tap_spk.shape)
            discrete_spk, vq_pen_spk, encoder_pen_spk, entropy_spk = self.vqspk(tap_spk.unsqueeze(2))
            discrete_spk = discrete_spk.squeeze(2)
            print("speaker VQ output before upsample (A)", discrete_spk.shape)
        self.train()
    ...
    def forward_spk_change(self, unpadded, padded, lang, vc_spk_idx):
        self.eval()
        with torch.no_grad() :
            continuous_phn = self.encoder(unpadded)
            continuous_spk = self.encoder_spk(padded)
            ...
            orig_phn, pen1, pen2, l1 = self.vq(continuous_phn.unsqueeze(2))
            orig_spk, pen3, pen4, l2 = self.vqspk(tap_output_spk.unsqueeze(2))
            all_spk_vq_vecs = self.vqspk.forward_codebook_normed(256, tap_output_spk.unsqueeze(2))
            print("orig_phn",orig_phn.shape)
            print("orig_spk",orig_spk.shape)
            print("all_spk",all_spk_vq_vecs.shape)
            print("all_spk[0]",all_spk_vq_vecs[0].shape)

            # squeeze 1 or 2?
            vc_spk = all_spk_vq_vecs[vc_spk_idx]
            print("vc_spk", vc_spk.shape)
            orig_global_spk = orig_spk.squeeze(2).squeeze(1)
            vc_global_spk = vc_spk.unsqueeze(0)
            print("orig_global_spk", orig_global_spk.shape)
            print("vc_global_spk", vc_global_spk.shape)
            print("lang", lang.shape)

            print("code_phn",orig_phn.shape)
            code_phn = orig_phn.squeeze(2)
            print("code_phn",code_phn.shape)
            global_codes = torch.cat((vc_global_spk, lang) , dim=1 )
#            global_codes = torch.cat((orig_global_spk, lang) , dim=1 )
            print("code_phn",code_phn.shape)
            print("global_codes", global_codes.shape)
            output = self.overtone.generate(code_phn, global_codes)
        return output


    def forward_spk_mix(self, unpadded, padded, lang, vc_spk_idx, vc_spk_idx2):
        self.eval()
        with torch.no_grad() :
            continuous_phn = self.encoder(unpadded)
            continuous_spk = self.encoder_spk(padded)
            ...
            # squeeze 1 or 2?
            vc_spk = all_spk_vq_vecs[vc_spk_idx]
            vc_spk2 = all_spk_vq_vecs[vc_spk_idx2]
            print("vc_spk", vc_spk.shape)
            orig_global_spk = orig_spk.squeeze(2).squeeze(1)
            vc_spk = vc_spk.unsqueeze(0)
            vc_spk2 = vc_spk2.unsqueeze(0)
            print("orig_global_spk", orig_global_spk.shape)
            print("vc_global_spk", vc_spk.shape)
            print("vc_global_spk2", vc_spk2.shape)
            print("lang", lang.shape)

            # mix the two speaker vectors using a mean
            vc_spk_stacked = torch.stack([vc_spk, vc_spk2])
            vc_global_codes = torch.mean(vc_spk_stacked, dim=0)
            code_phn = orig_phn.squeeze(2)
            print("code_phn",code_phn.shape)
            global_codes = torch.cat((vc_global_codes, lang) , dim=1 )
            print("global_codes", global_codes.shape)
            output = self.overtone.generate(code_phn, global_codes)
        return output
    ...
    def forward_spk_change_debug(self, unpadded, padded, lang, vc_spk_idx):
        self.eval()
        with torch.no_grad() :
            continuous_phn = self.encoder(unpadded)
            continuous_spk = self.encoder_spk(padded)
            tap_output_spk = self.tap_spk(continuous_spk)
            true_spk_code = self.vqspk.forward_custom(tap_output_spk.unsqueeze(2))
            print(true_spk_code)
            
            orig_phn, pen1, pen2, l1 = self.vq.forward_gen(continuous_phn.unsqueeze(2))
            orig_spk, pen3, pen4, l2 = self.vqspk.forward_gen(tap_output_spk.unsqueeze(2))
            all_spk_vq_vecs = self.vqspk.forward_codebook_normed(256, tap_output_spk.unsqueeze(2))
            print("full codebook shape", all_spk_vq_vecs.shape)
            retrieved_spk = all_spk_vq_vecs[true_spk_code]
            vc_spk = all_spk_vq_vecs[vc_spk_idx]
           
            orig_spk = orig_spk.squeeze(2).squeeze(1).squeeze(0)
            vc_spk = vc_spk.unsqueeze(0).squeeze(0)
            retrieved_spk = retrieved_spk.squeeze(0).squeeze(0)
            print(orig_spk[:5])
            print(retrieved_spk[:5])
           
        return

    

    def forward_spk_change2(self, orig_unpadded, orig_padded, target_unpadded, target_padded, lang):
        self.eval()
        with torch.no_grad() :
            orig_continuous_phn = self.encoder(orig_unpadded)
            orig_continuous_spk = self.encoder_spk(orig_padded)
            orig_tap_output_spk = self.tap_spk(orig_continuous_spk)
            orig_phn, pen1, pen2, l1 = self.vq(orig_continuous_phn.unsqueeze(2))
            orig_spk, pen3, pen4, l2 = self.vqspk(orig_tap_output_spk.unsqueeze(2))
            print("orig_phn",orig_phn.shape)
            print("orig_spk",orig_spk.shape)
            target_continuous_phn = self.encoder(target_unpadded)
            target_continuous_spk = self.encoder_spk(target_padded)
            target_tap_output_spk = self.tap_spk(target_continuous_spk)
            target_phn, pen1, pen2, l1 = self.vq(target_continuous_phn.unsqueeze(2))
            target_spk, pen3, pen4, l2 = self.vqspk(target_tap_output_spk.unsqueeze(2))

            orig_global_spk = orig_spk.squeeze(2).squeeze(1)
            vc_global_spk = target_spk.squeeze(2).squeeze(1)
            print("orig_global_spk", orig_global_spk.shape)
            print("vc_global_spk", vc_global_spk.shape)
            print("lang", lang.shape)
    ...
    def do_train(self, paths, dataset, optimiser, epochs, batch_size, num_workers, step, train_sampler, device, lr=1e-4,  spk_lr=0.01, valid_index=[], use_half=False, do_clip=False):
        ...
        for e in range(epochs) :
            ...
            for i, (speaker, lang, wave16) in enumerate(trn_loader) :
                ...
                # compare speaker predicted with truth
                # print(speaker.shape, "speaker")
                spk_true = torch.max(speaker, 1)[1]
#                print("predicted", spk_predict)
#                print("truth", spk_true)
#                print("predicted", spk_predict.shape)
#                print("truth", spk_true.shape)
                spk_loss1 = spk_criterion(spk_predict1, spk_true)
#                print("spk_loss1", spk_loss1)
...
# add speaker classifier cross entropy loss to this loss function
...
f'-- Lc={max_loss_c:#.4} -- Lf={max_loss_f:#.4} -- Lvq={max_loss_vq:#.4} -- Lvqc={max_loss_vqc:#.4} -- Lvqcw={max_loss_vqcw:#.4} -- LvqS={max_loss_vq_spk:#.4} -- LvqcS={max_loss_vqc_spk:#.4} -- LvqcwS={max_loss_vqcw_spk:#.4}')
                ...
                # Debugging validate after one batch
...
#                sys.exit()
    ...
    def validate2(self, paths, step, data_path, test_index, deterministic=False, use_half=False, verbose=False):
        k = step // 1000
        os.makedirs(paths.gen_path(), exist_ok=True)
        criterion = nn.NLLLoss().cuda()        
        batch_size = 50
        dataset = env.MultispeakerDataset_lang(test_index, data_path)
        test_loader = DataLoader(dataset, collate_fn=self.tmp_func, batch_size=batch_size, pin_memory=True)
        iters = len(test_loader)        
        running_loss_c = 0.
        running_loss_f = 0.
        pad_left = self.pad_left()
        ...
        pad_left_decoder = self.pad_left_decoder()
        pad_right = self.pad_right()
        for i, (speaker, lang, wave16) in enumerate(test_loader) :
            wave16 = wave16.cuda()
            speaker = speaker.long().cuda()
            lang = lang.long().cuda()
            coarse = (wave16 + 2**15) // 256
            ...
            out = self.forward_validate2(x, translated, lang)
            p_c, p_f = out
            y_coarse = coarse[:, pad_left+1:1-pad_right]
            y_fine = fine[:, pad_left+1:1-pad_right]
            running_loss_c += criterion(p_c.transpose(1, 2).float(), y_coarse)
            running_loss_f += criterion(p_f.transpose(1, 2).float(), y_fine)
        return running_loss_c/iters, running_loss_f/iters
